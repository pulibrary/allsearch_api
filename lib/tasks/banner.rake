...
namespace :banner do
  ...
  task :update, [:text, :alert_status, :dismissible, :autoclear] => [:autoload, :database_connection] do |_task, args|
    banner_repo = RepositoryFactory.new(ALLSEARCH_ROM).banner
    banner_repo.modify args.to_h
    Rake::Task['banner:show'].invoke
  end

  desc 'Show current banner configuration'
  task show: [:autoload, :database_connection] do
    banner_repo = RepositoryFactory.new(ALLSEARCH_ROM).banner
