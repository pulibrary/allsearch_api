...
namespace :solr do
  ...
  task :load_sample_data_file, [:service, :filename] => :lando do |_task, args|
  ...
  task :create_sample_data, [:query, :solr_collection, :filename] => :autoload do |_task, args|
    args.with_defaults(query: 'cats', solr_collection: 'catalog-staging', filename: 'catalog.json')
    ...
    puts '`ssh -L 7872:localhost:8983 deploy@lib-solr-staging2.princeton.edu`'
  end
end
